...
def _pick_winner(labels: List[str], counts: Counter) -> Tuple[str, str]:
    """
    Given a list of labels known to be equivalent, pick the canonical winner.
    Strategy: prefer known alias → then most frequent → then longest (most specific).
    ...
    # Most frequent
    by_freq = sorted(labels, key=lambda l: (-counts[l], -len(l)))
    return by_freq[0], "frequency"
...
def _normalize_field(
    ...
    for ckey, group in canonical_groups.items():
        winner, reason = _pick_winner(group, counts)
